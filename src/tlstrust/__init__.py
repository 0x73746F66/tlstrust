...
class TrustStore:
    ...
    def certificate(self) -> X509:
        ...
        for _, context_type in STORES.items():
            try:
                certificate = get_certificate_from_store(self.key_identifier, context_type)
    ...
    def android(self) -> bool:
        untrusted = set(ANDROID_UNTRUSTED + ANDROID2_2_UNTRUSTED + ANDROID2_3_UNTRUSTED + ANDROID3_UNTRUSTED + ANDROID4_UNTRUSTED + ANDROID4_4_UNTRUSTED + ANDROID7_UNTRUSTED + ANDROID8_UNTRUSTED + ANDROID9_UNTRUSTED + ANDROID10_UNTRUSTED + ANDROID11_UNTRUSTED + ANDROID12_UNTRUSTED)
    ...
    def exists(self, context_type :int) -> bool:
        ...
        if context_type == SOURCE_CCADB and self.key_identifier in CCADB_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_CCADB))
        if context_type == SOURCE_JAVA and self.key_identifier in JAVA_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_JAVA))
        if context_type == SOURCE_ANDROID and self.key_identifier in ANDROID_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_ANDROID))
        if context_type == SOURCE_LINUX and self.key_identifier in LINUX_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_LINUX))
        if context_type == SOURCE_RUSSIA and self.key_identifier in RUSSIA_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_RUSSIA))
        if context_type == SOURCE_CERTIFI and self.key_identifier in CERTIFI_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, SOURCE_CERTIFI))
        if context_type == PLATFORM_ANDROID12 and self.key_identifier in ANDROID12_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID12))
        if context_type == PLATFORM_ANDROID11 and self.key_identifier in ANDROID11_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID11))
        if context_type == PLATFORM_ANDROID10 and self.key_identifier in ANDROID10_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID10))
        if context_type == PLATFORM_ANDROID9 and self.key_identifier in ANDROID9_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID9))
        if context_type == PLATFORM_ANDROID8 and self.key_identifier in ANDROID8_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID8))
        if context_type == PLATFORM_ANDROID7 and self.key_identifier in ANDROID7_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        if context_type == PLATFORM_ANDROID4_4 and self.key_identifier in ANDROID4_4_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        if context_type == PLATFORM_ANDROID4 and self.key_identifier in ANDROID4_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        if context_type == PLATFORM_ANDROID3 and self.key_identifier in ANDROID3_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        if context_type == PLATFORM_ANDROID2_3 and self.key_identifier in ANDROID2_3_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        if context_type == PLATFORM_ANDROID2_2 and self.key_identifier in ANDROID2_2_PEM_FILES.keys():
            return match_certificate(self.key_identifier, get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID7))
        return False
    ...
    def expired_in_store(self, context_type :int) -> bool:
        ...
        return get_certificate_from_store(self.key_identifier, context_type=context_type).has_expired()
...
def trust_stores_from_chain(certificates: list[X509]) -> list[TrustStore]:
    ...
    if not isinstance(leaf, X509):
        raise InvalidChainError('certificate chain is empty or missing a server leaf certificate')
